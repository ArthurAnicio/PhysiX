...
const Invite: React.FC<InviteProps> = ({ invite, sync }) => {
    ...
    const [schedule,setSchedule] = useState<Schedule>(invite.schedule);
    ...
    function formatSchedule(){
        switch (schedule.week_day){
            case '0':
                return setSchedule({week_day:'Domingo', from: schedule.from, to: schedule.to,id: schedule.id});
            case '1':
                return setSchedule({week_day:'Segunda', from: schedule.from, to: schedule.to,id: schedule.id});
            case '2':
                return setSchedule({week_day:'Terça', from: schedule.from, to: schedule.to,id: schedule.id});
            case '3':
                return setSchedule({week_day:'Quarta', from: schedule.from, to: schedule.to,id: schedule.id});
            case '4':
                return setSchedule({week_day:'Quinta', from: schedule.from, to: schedule.to,id: schedule.id});
            case '5':
                return setSchedule({week_day:'Sexta', from: schedule.from, to: schedule.to,id: schedule.id});
            case '6':
                return setSchedule({week_day:'Sábado', from: schedule.from, to: schedule.to,id: schedule.id});
            default:
                return '';
        }
    }
