...
function LogInTeacher() {
  ...
  async function validateLogin() {
    if (username && password) {
      try {
        const response = await api.get("/teacher-login", {
        ...
        if (response.status === 200) {
          if (response.data.verified == 1) {
            ...
            history("/teacher_area", {
              state: { teacherId: response.data.id },
            });
          } else {
            history("/unverified_teacher", {
      ...
      <div id={styles.loginContainer}>
        <div id={styles.loginWindow}>
          ...
          <section id={styles.inputsMain}>
            ...
            />
            <Link to="/sign_up_teacher" id={styles.purpleLink}>
              Crie uma conta
            </Link>
          </section>
        </div>
